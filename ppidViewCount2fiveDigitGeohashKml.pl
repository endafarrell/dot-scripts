...
while (<>) {
	chomp;
	my $geoH = substr($_, 3, $KML_GEOHASH_LEN);	
	my @csv = split(/\,/, $_);
	my $views = $csv[1];
	if ( exists $uniqueGeohashs{$geoH} ){
		$uniqueGeohashs{$geoH} = $uniqueGeohashs{$geoH} + $views;
	} else {
		$uniqueGeohashs{$geoH} = $views;
	};
...
for my $geoHash ( sort keys %uniqueGeohashs ){
	...
	print polygon("$geoHash - $num", $lat, $lng, 
		$latrange->[0],$lngrange->[0], 
		$latrange->[1], $lngrange->[1], $num);
}
print kml_footer();
